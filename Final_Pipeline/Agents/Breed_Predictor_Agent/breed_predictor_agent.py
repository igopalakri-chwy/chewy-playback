...
class BreedPredictorAgent:
    ...
    def should_predict_breed(self, pet_data: Dict[str, Any]) -> bool:
        ...
        # Check if breed is unknown or mixed
        breed = pet_data.get('Breed', pet_data.get('breed', '')).lower()
        unknown_indicators = ['unknown', 'unk', 'mixed', 'mix', 'mutt', 'unknown breed', '']
        
        return any(indicator in breed for indicator in unknown_indicators)
